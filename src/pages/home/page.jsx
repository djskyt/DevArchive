...
export const HomePage = () => {
  const { query, setQuery, articles, isPending, isError } = useSearch();

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">아티클</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">개발 아티클을 저장하고 관리하세요</p>
      </div>
      ...
      {isError && (
        <p className="text-sm text-red-400 text-center py-10">아티클을 불러올 수 없습니다.</p>
      )}
